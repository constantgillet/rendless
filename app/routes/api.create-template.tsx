...
import { publicTemplates } from "~/constants/publicTemplates";
...
export const validator = withZod(
	z.object({
		...
		fromPublicTemplate: z.string().optional(),
	}),
);
...
export async function action({ context, request }: ActionFunctionArgs) {
	...
	if (result.data.fromPublicTemplate) {
		const template = publicTemplates.find(
			(template) => template.name === result.data.fromPublicTemplate,
		);

		duplicatedTemplateTree = template.tree ?? null;
	}
